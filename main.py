...
if hex(getnode()) in ["0xdc7b23bb434e"]:  # windows masaüstü pc ise veya laptop
    ...
    print("kyb")
    bot_adi = "@seragamebot"
    bot_token = "6118272326:AAEy93_2wLjFdCKycRXbOww3bQDXcolX3rg"
...
async def log_gonder(**kwargs):
    ...
    try:
        await bot.send_message(-1001989915824, f"""
<b> ~~ 📢 Log Kaydı ~~</b>
...
Eylem: <code>{kwargs.get('eylem','')}</code>
    ...
    """, disable_web_page_preview=True)
        ...
        #    bot.send_message(kurucu_id, str(e))
    try:
        await bot.set_chat_title(-1001989915824, f"Bot Log - {len(oyunlar)}")
    except Exception as e:
        if "chat not found" in str(e):
            pass
...
async def sessiz_sinema_baslat(message, **kwargs):
    ...
    if user_id in admins and user_id != 1449935113 and user_id != 1449935113:
        ayir = text.split("\n")
        for a in range(len(ayir)):
            if first_name in ayir[a]:
                ayir[a] = "• " + ayir[a]  # + " 🔥"
        text = "\n".join(ayir)
    elif user_id == 1449935113 or user_id == 1449935113:
        ayir = text.split("\n")
        for a in range(len(ayir)):
            if first_name in ayir[a]:
                ayir[a] = "🏅 " + ayir[a]  # + " 🔥"
    ...
    callback_button = types.InlineKeyboardButton(
        text="Kendi Kelimem 📝", url=f"https://t.me/SeraGameBot?start={oyun_id}")
...
async def baslat(message):
    ...
    keyboard = types.InlineKeyboardMarkup()
    callback_button1 = types.InlineKeyboardButton(
        text="✍️ Kelime Oyunu", callback_data="sessiz_sinema")
    callback_button2 = types.InlineKeyboardButton(
...
async def callback_inline(cagri):  # çağrıcı cagrici
    ...
    if "group" in msg_type:
        ...
        if f(f"groups.{chat_id}.username") == "":
            await bot.send_message(-1001989915824, f"📜 {grup_username} ⟶ {len(f('groups')) + 1}")
...
async def handle_callback_query(call):
    cagri = call
    sorgu = call.data.split("_")[0]
    oyun_id = call.data.split("_")[1]
    chat_id = call.message.chat.id
    user_id = call.from_user.id
    first_name = call.from_user.first_name
    zaman_hassasiyeti = 1000000000
    if sorgu == "kelime_bak":
        # def yap():
        kelime = f(f"games.{oyun_id}.kelime")
        txt = "📖 Sorun: "+kelime + "\n\n"
        ...
        user_messages = ""
        messages = await bot.history(chat_id, limit=100)
        for message in messages:
            if message.from_user.id == acan_user:
                user_messages += "\n- " + message.text
                
        if user_messages:
            await bot.send_message(chat_id, f'İşte {acan_user} tarafından yazılan mesajlar: {user_messages}')
        else:
            await bot.answer_callback_query(cagri.id, f'{acan_user} henüz bir mesaj yazmadı.', show_alert=False)
